...
//Setup the spinner's radius and length
...
//Configurate the goolgle map module by providing version, libraries being used and API key
...
//Configurate the angular bootstrap valdation module
...
/*
The page routing uses ui-router: https://ui-router.github.io/ng1/
A webpage has been divided several regions and each region layout is called a state.
A page redirection is achieved by a state change.

resolve: data to be obtained *before* rendering the page
onEnter: steps to go through *after* rendering the page


All states should be defined here. State hiearchy:
-auth: states before authentication
--auth.home: homepage
--auth.laSearch: local advisor search page
--auth.advisorDetail: local advisor homepage
--auth.messenger: chat page
---auth.messenger.chatpanel: chat page with specific target user
--auth.locRec: local recommendation search
--auth.recCreation: local recommnedation creation
--auth.recDetail: recommendation detail page
--auth.editProfile: edit profile page

-unauth: states after authentication
--unauth.home: homepage
--unauth.laSearch: local advisor search page
--unauth.advisorDetail: local advisor homepage
--unauth.locRec: local recommendation search page
--unauth.recDetail: local recommendation detail page
*/
        ...
        .state('unauth.home', {
            ...
            resolve: {
                localAdvisors: function(searchHelper) {
                    return searchHelper.searchLocalAdvisors({
                        "limit": 3,
                        "request_fields": ["id", "first_name", "last_name", "profile_photo_url", "local_advisor_profile"]
                    })
                },
                recommendations: function(searchHelper) {
                    return searchHelper.searchRecommendations({
                        "limit": 3,
                        "request_fields": ["id", "title", "description", "primary_picture"]
                    })
                },
...
app.run(function($rootScope, $state, AuthService, utils) {
    ...
    $rootScope.$on("$stateChangeStart", function(event, toState, toParams, fromState, fromParams) {
        utils.requestStart()
        //if the state to go is not same as before, we should do the checking. Otherwise we can allow the jump.
        if (!angular.equals($rootScope.toState, toState) || !angular.equals($rootScope.toParams, toParams)) {
            //firstly, stop the jump event
            event.preventDefault();

            //record the jump target this time
            $rootScope.toState = toState
            $rootScope.toParams = toParams

            /*
            Check if the user is logged in.
            -logged in: if state belongs to unauth, redirect to corresponding auth state
            -nog logged in: if state belongs to auth, redirect to corresponding unauth state
            */
...
app.filter('dateFormatter', function() { // filter is a factory function
    return function(unformattedDate, emptyStrText) { // first arg is the input, rest are filter params
        //If the date is a dataobject, use London time. Or just use the default
